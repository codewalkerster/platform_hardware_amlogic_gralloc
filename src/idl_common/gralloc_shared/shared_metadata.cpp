...
struct shared_metadata
{
	...
	int32_t am_omx_tunnel{0};
	int32_t am_omx_flag{0};
	int32_t am_omx_video_type{0};
	int32_t am_omx_buffer_sequence{0};
#endif
...
#ifdef GRALLOC_AML_EXTEND
static_assert(offsetof(shared_metadata, am_omx_tunnel) == 6476, "bad alignment");
static_assert(sizeof(shared_metadata::am_omx_tunnel) == 4, "bad size");

static_assert(offsetof(shared_metadata, am_omx_flag) == 6480, "bad alignment");
static_assert(sizeof(shared_metadata::am_omx_flag) == 4, "bad size");

static_assert(offsetof(shared_metadata, am_omx_video_type) == 6484, "bad alignment");
static_assert(sizeof(shared_metadata::am_omx_video_type) == 4, "bad size");

static_assert(offsetof(shared_metadata, am_omx_buffer_sequence) == 6488, "bad alignment");
...
static_assert(sizeof(shared_metadata) == 6496, "bad size");
#else
static_assert(alignof(shared_metadata) == 8, "bad alignment");
...
#ifdef GRALLOC_AML_EXTEND
void get_omx_tunnel(const imported_handle *hnd, int32_t *am_omx_tunnel)
{
	auto *metadata = reinterpret_cast<const shared_metadata *>(hnd->attr_base);
	*am_omx_tunnel = metadata->am_omx_tunnel;
}

void set_omx_tunnel(const imported_handle *hnd, const int32_t am_omx_tunnel)
{
	auto *metadata = reinterpret_cast<shared_metadata *>(hnd->attr_base);
	metadata->am_omx_tunnel = am_omx_tunnel;
}

void get_omx_flag(const imported_handle *hnd, int32_t *am_omx_flag)
{
	auto *metadata = reinterpret_cast<const shared_metadata *>(hnd->attr_base);
	*am_omx_flag = metadata->am_omx_flag;
}

void set_omx_flag(const imported_handle *hnd, const int32_t am_omx_flag)
{
	auto *metadata = reinterpret_cast<shared_metadata *>(hnd->attr_base);
	metadata->am_omx_flag = am_omx_flag;
}

void get_omx_video_type(const imported_handle *hnd, int32_t *am_omx_video_type)
{
	auto *metadata = reinterpret_cast<const shared_metadata *>(hnd->attr_base);
	*am_omx_video_type = metadata->am_omx_video_type;
}

void set_omx_video_type(const imported_handle *hnd, const int32_t am_omx_video_type)
{
	auto *metadata = reinterpret_cast<shared_metadata *>(hnd->attr_base);
	metadata->am_omx_video_type = am_omx_video_type;
}

void get_omx_buffer_sequence(const imported_handle *hnd, int32_t *am_omx_buffer_sequence)
{
	auto *metadata = reinterpret_cast<const shared_metadata *>(hnd->attr_base);
	*am_omx_buffer_sequence = metadata->am_omx_buffer_sequence;
}

void set_omx_buffer_sequence(const imported_handle *hnd, const int32_t am_omx_buffer_sequence)
{
	auto *metadata = reinterpret_cast<shared_metadata *>(hnd->attr_base);
