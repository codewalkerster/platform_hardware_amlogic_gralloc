...
public:
	...
	AIMapper_Error lock(buffer_handle_t _Nonnull buffer, uint64_t cpuUsage, ARect accessRegion, int acquireFence,
	                    void *_Nullable *_Nonnull outData) override;
	...
	int32_t getMetadata(buffer_handle_t _Nonnull buffer, AIMapper_MetadataType metadataType, void *_Nonnull outData,
	                    size_t outDataSize) override;

	int32_t getStandardMetadata(buffer_handle_t _Nonnull buffer, int64_t standardMetadataType, void *_Nonnull outData,
	                            size_t outDataSize) override;
	...
	AIMapper_Error setStandardMetadata(buffer_handle_t _Nonnull buffer, int64_t standardMetadataType,
	                                   const void *_Nonnull metadata, size_t metadataSize) override;
	...
	AIMapper_Error getReservedRegion(buffer_handle_t _Nonnull buffer, void *_Nullable *_Nonnull outReservedRegion,
	                                 uint64_t *_Nonnull outReservedSize) override;

private:
	std::vector<AIMapper_MetadataTypeDescription> m_stablec_metadata;
};
