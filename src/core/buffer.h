...
//meson graphics changes start
	...
	uint32_t req_width{0};
	uint32_t req_height{0};
...
//meson graphics changes end
	...
	private_handle_t(int in_flags, int in_size, uint64_t in_consumer_usage, uint64_t in_producer_usage, int in_shared_fd,
	                 ...
	                 int in_layer_count, const plane_layout &in_plane_info, int in_stride, bool in_is_multi_plane)
	    ...
	    , is_multi_plane_internal{ in_is_multi_plane }
	    , stride{ in_stride }
	...
	{
		return (is_multi_plane_internal);
	}
	...
	{
		return is_multi_plane() ? (plane_info[2].offset == 0 ? 2 : 3) : 1;
	}
};
...
struct imported_handle : private_handle_t
{
	...
	std::atomic<bool> invalid_write_locks{};
...
unique_private_handle make_private_handle(int flags, int size, uint64_t consumer_usage,
