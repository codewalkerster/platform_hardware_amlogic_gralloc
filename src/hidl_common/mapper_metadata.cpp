...
static AmlMetadataType getAmlMetadataTypeValue(const MetadataType& metadataType) {
	return static_cast<AmlMetadataType>(metadataType.value);
}
...
void get_metadata(const private_handle_t *handle, const IMapper::MetadataType &metadataType, IMapper::get_cb hidl_cb)
{
	...
	else if (isAmlMetadataType(metadataType))
	{
		...
		switch (getAmlMetadataTypeValue(metadataType))
		{
